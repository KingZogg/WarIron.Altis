//Main Airport
_pos = [14605.230469,16772.962891,0.0191288];
_object = createVehicle ["Land_Atm_02_F", _pos, [], 0, "CAN_COLLIDE"];
_object setDir 225.001;
_object setPosATL _pos;

_pos = [14609.18457,16767.867188,3.7];
_object = createVehicle ["Flag_Suatmm_F", _pos, [], 0, "CAN_COLLIDE"];
_object setDir 340;
_object setPosATL _pos;



//Agois
_pos = [9278.635742,15845.688477,0.251045];
_object = createVehicle ["Land_Atm_02_F", _pos, [], 0, "CAN_COLLIDE"];
_object setDir 160;
_object setPosATL _pos;

_pos = [9283.635742,15846.182617,7.93826];
_object = createVehicle ["Flag_Suatmm_F", _pos, [], 0, "CAN_COLLIDE"];
_object setDir 140;
_object setPosATL _pos;

_pos = [9272.396484,15862.791992,0.199081];
_object = createVehicle ["Flag_Blueking_F", _pos, [], 0, "CAN_COLLIDE"];
_object setDir 0;
_object setPosATL _pos;



//Kavala
//_pos = [3654.0268555,12822.479492,0.187351];
//_object = createVehicle ["Land_Atm_02_F", _pos, [], 0, "CAN_COLLIDE"];
//_object setDir 343;
//_object setPosATL _pos;

//_pos = [3660.200195,12809.59082,18.8929];
//_object = createVehicle ["Flag_Suatmm_F", _pos, [], 0, "CAN_COLLIDE"];
//_object setDir 0;
//_object setPosATL _pos;



//Pyrgos
//_pos = [16898.941406,12703.691406,0.0353642];
//_object = createVehicle ["Land_Atm_02_F", _pos, [], 0, "CAN_COLLIDE"];
//_object setDir 222.101;
//_object setPosATL _pos;

//_pos = [16901.996094,12700.0742188,8.22854];
//_object = createVehicle ["Flag_Suatmm_F", _pos, [], 0, "CAN_COLLIDE"];
//_object setDir 0;
//_object setPosATL _pos;



//north east vehicle shop
//_pos = [22504.570313,18607.626953,-0.0585299];
//_object = createVehicle ["Land_Atm_02_F", _pos, [], 0, "CAN_COLLIDE"];
//_object setDir 131;
//_object setPosATL _pos;

//_pos = [22505.167969,18606.929688,5.58331];
//_object = createVehicle ["Flag_Suatmm_F", _pos, [], 0, "CAN_COLLIDE"];
//_object setDir 131;
//_object setPosATL _pos;



//Selakano
/*
_pos = [20818.402344,6796.648438,0.264572];
_object = createVehicle ["Land_Atm_02_F", _pos, [], 0, "CAN_COLLIDE"];
_object setDir 213;
_object setPosATL _pos;

_pos = [20816.5,6798.110352,8.12192];
_object = createVehicle ["Flag_Suatmm_F", _pos, [], 0, "CAN_COLLIDE"];
_object setDir 0;
_object setPosATL _pos;
*/
_pos = [20790.0234375,6802.788086,7.46657];
_object = createVehicle ["Flag_Blueking_F", _pos, [], 0, "CAN_COLLIDE"];
_object setDir 0;
_object setPosATL _pos;


//Oreokastro
_pos = [4570.996094,21381.458984,6.4303];
_object = createVehicle ["Flag_Suatmm_F", _pos, [], 0, "CAN_COLLIDE"];
_object setDir 0;
_object setPosATL _pos;


_pos = [4572.615234,21380.273438,0.0835571];
_object = createVehicle ["Land_Atm_02_F", _pos, [], 0, "CAN_COLLIDE"];
_object setDir 220.701;
_object setPosATL _pos;


_pos = [4562.0722656,21385.537109,0.257935];
_object = createVehicle ["Land_Communication_F", _pos, [], 0, "CAN_COLLIDE"];
_object setDir 310;
_object setPosATL _pos;



//Charkia
_pos = [18094.0898438,15244.682617,7.36176];
_object = createVehicle ["Flag_Blueking_F", _pos, [], 0, "CAN_COLLIDE"];
_object setDir 0;
_object setPosATL _pos;



//Vikos
_pos = [11527.955078,9441.9375,7.46946];
_object = createVehicle ["Flag_Blueking_F", _pos, [], 0, "CAN_COLLIDE"];
_object setDir 0;
_object setPosATL _pos;


//Kore Territory C Tower
_pos = [6924.546387,15911.490234,0];
_object = createVehicle ["Land_Cargo_Tower_V1_F", _pos, [], 0, "CAN_COLLIDE"];
_object setDir 0;
_object setPosATL _pos;



//edessa

_pos = [7171.522949,11058.269531,0.108559];
_object = createVehicle ["Land_Cargo_HQ_V1_F", _pos, [], 0, "CAN_COLLIDE"];
_object setDir 16;
_object setPosATL _pos;


_pos = [7178.208008,11038.111328,0.458144];
_object = createVehicle ["Land_LampStreet_off_F", _pos, [], 0, "CAN_COLLIDE"];
_object setDir 15.9998;
_object setPosATL _pos;


_pos = [7114.121582,11047.584961,-0.3003];
_object = createVehicle ["Land_Cargo_Tower_V1_No1_F", _pos, [], 0, "CAN_COLLIDE"];
_object setDir 194;
_object setPosATL _pos;


_pos = [7176.157715,11044.121094,0.000643253];
_object = createVehicle ["Land_BarGate_F", _pos, [], 0, "CAN_COLLIDE"];
_object setDir 283.2;
_object setPosATL _pos;


_pos = [7174.62207,11111.945313,0.760638];
_object = createVehicle ["Land_Atm_02_F", _pos, [], 0, "CAN_COLLIDE"];
_object setDir 27.5999;
_object setPosATL _pos;


_pos = [7172.325684,11113.194336,0.156637];
_object = createVehicle ["Land_LampStreet_small_F", _pos, [], 0, "CAN_COLLIDE"];
_object setDir 28.5999;
_object setPosATL _pos;


_pos = [7174.333008,11113.918945,4.80176];
_object = createVehicle ["Flag_Suatmm_F", _pos, [], 0, "CAN_COLLIDE"];
_object setDir 0;
_object setPosATL _pos;


_pos = [7182.589844,11109.78125,4.80429];
_object = createVehicle ["Flag_Blueking_F", _pos, [], 0, "CAN_COLLIDE"];
_object setDir 0;
_object setPosATL _pos;

_pos = [7178.465332,11126.793945,0.209508];
_object = createVehicle ["Land_Communication_F", _pos, [], 0, "CAN_COLLIDE"];
_object setDir 300;
_object setPosATL _pos;


//kavlava hospital heli

_pos = [3732.499756,12976.683594,19.5519];
_object = createVehicle ["O_Heli_Transport_04_medevac_F", _pos, [], 0, "CAN_COLLIDE"];
_object setDir 270;
_object setPosATL _pos;























