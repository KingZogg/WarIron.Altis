...
if (_alivePlayerCount < 1) exitWith {};
...
[format ["addBounty:%1:%2:%3:%4:%5:%6:%7:%8", call A3W_extDB_ServerID, call A3W_extDB_MapID, 'SYSTEM', getPlayerUID _foundPlayer, '0', _mission_state, 10000, 80000]] call extDB_Database_async;
...
if (_currTime - _startTime >= bountyMissionTimeout) then { _mission_state = BOUNTY_MISSION_END_SURVIVED; };
	...
	((_mission_state == BOUNTY_MISSION_END_TEAMKILLED) || (_mission_state == BOUNTY_MISSION_END_SURVIVED) || (_mission_state == BOUNTY_MISSION_END_DISCONNECT))
...
_failedExec =
{
	...
	if (_mission_state == BOUNTY_MISSION_END_SURVIVED) then {
		...
		_playerMoney = _playerMoney + 80000;
	...
	if (_mission_state == BOUNTY_MISSION_END_DISCONNECT) then {
		_failedHintMessage =  format ["<t align='center' color='%2' shadow='2' size='1.75'>Objective Failed</t><br/><t align='center' color='%2'>------------------------------</t><br/><t align='center' color='%3' size='1.25'>PUSSY!!</t><br/><br/><t align='center' color='%3'>%1 took the coward's way out and suicided or disconnected! He will become the bounty when he reconnects!</t>", _playerName, failMissionColor, subTextColor];
		[format ["updateBounty:%1:%2:%3:%4", _destPlayerUID, '0','0', _mission_state]] call extDB_Database_async;
	};
